            ...
            if zilch:
                ...
                if response == "q":
                    ...
                    if input_is_valid is False:
                        ...
                        if response == "q":
                            print(
                                f"Thanks for playing. You earned {self.bank.balance} points"
                            )
                            sys.exit()
                        else:
                            die = die - len(dice_to_keep)
                            local_total += GameLogic.calculate_score(
                                tuple(dice_to_keep)
                            )
                            print(
                                f"You have {local_total} unbanked points and {die} dice remaining"
                            )
                            ...
                            if response == "r":
                                ...
                                if zilch:
                                    ...
                                    dice_to_keep = [int(x) for x in str(response)]
                                    input_is_valid = GameLogic.validate_keepers(
                                        roll, dice_to_keep
                                    )
                                    if input_is_valid is False:
                                        print(f"*** {roll_input} ***")
                                ...
                                print(
                                    f"You banked {local_total} points in round {round_num}"
                                )
